...
in
{
  options.services.mautrix-discord = {
    enable = lib.mkEnableOption (
      lib.mdDoc "mautrix-discord, a puppeting/relaybot bridge between Matrix and Discord."
    );
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.mautrix-discord.settings = lib.mkMerge (
      map mkDefaults [
        defaultConfig
  ...
  meta.maintainers = with lib.maintainers; [ lykos153 ];
}
